; (function () {
  ...
  function getItemImage(item) {
    ...
    if (shape.includes('trụ dài')) return '../img/trụ dài.webp'
    if (shape.includes('trụ ngắn')) return '../img/trụ ngắn.jpg'
    if (shape.includes('hình hộp')) return '../img/hình hộp.webp'
    return '../img/6.jpg'
  }
  ...
  function renderCard(item) {
    ...
    return `
      <a href="${href}" class=""><div class="card">
        <div class="thumb">
        ...
        <div class="actions">Xem chi tiết<span class="badge">${item.certificate || '—'}</span></div>
      </div></a>
    `
  }
  ...
  function renderDetailSample(sample) {
    ...
    // Hiển thị danh sách kho & số lượng dưới dạng bảng chi tiết
    const inv = (sample.inventory || []).map(x => {
      const w = findWarehouse(x.warehouseId)
      return `
      <tr>
        <td>${w?.name || x.warehouseId}</td>
    ...
    return `
      <div class="panel">
        <div class="detail-thumb"><img src="${img}" alt="${sample.name}" class="image-width"></div>
        <h2>${sample.name}</h2>
